...
function QuizRewardPicker() {
  const { quizResourcePickRemaining, pickQuizReward, dismissQuiz } = useGameStore();
        ...
        {resourceOrder.map(res => {
          ...
          return (
            <motion.button
              ...
              onClick={() => {
                pickQuizReward(res);
                if (quizResourcePickRemaining <= 1) {
                  setTimeout(() => dismissQuiz(), 300);
                }
              }}
...
export default function QuizPopup() {
  const {
    phase, currentQuiz, quizResult, quizResourcePickRemaining,
    handleQuizAnswer, handleQuizTimeout, dismissQuiz,
  } = useGameStore();
  ...
  // Reset state when a new quiz appears
  if (currentQuiz && currentQuiz.id !== lastQuizId) {
    setLastQuizId(currentQuiz.id);
    setIsFlipped(false);
  ...
  }, [quizTimeRemaining, quizResult, isFlipped, handleQuizTimeout]);
  ...
  console.log('[QuizPopup] phase=', phase, 'currentQuiz=', currentQuiz?.id, 'quizResult=', quizResult);
